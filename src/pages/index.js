...
function Index() {
   ...
   const handleClick = async () => {
      ...
      if (isLoggedIn) {
         ...
         const unsub = onSnapshot(collection(db, 'users', userId, 'bookmarks'), (snapshot) => {
   ...
   return (
      <>
         {load ? (
            <div className='flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 bg-gradient-to-tr from-[#0a0f1c] via-[#101a2b] to-[#1a2236] min-h-screen'>
                  ...
                  <div className='mt-16 flex flex-col md:flex-row gap-8 items-center justify-center w-full max-w-5xl mx-auto'>
                     {[
                        ...
                        >
                           ...
                           <h3 className='font-bold text-lg mb-1 text-cyan-700 dark:text-cyan-300'>{f.title}</h3>
                           <p className='text-gray-700 dark:text-gray-200 text-base'>{f.desc}</p>
                        </motion.div>
                     ))}
                  </div>
